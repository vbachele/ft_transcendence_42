...
export class Pong {
	public readonly engine: Matter.Engine = Engine.create({gravity: {scale: 0}});
	public readonly world: Matter.World = this.engine.world;
	public readonly runner: Matter.Runner = Runner.create();
	public readonly detector: Matter.Detector = Detector.create();
	...
	constructor(dispatchToLobby: (event: string, data: any) => void) {
		...
		this.createBall();
		this.createPaddles();
		Detector.setBodies(this.detector, [
			this.ball,
